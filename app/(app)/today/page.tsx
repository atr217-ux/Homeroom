...
export default function TodayPage() {
  ...
  async function saveTaskEdit(id: string) {
    ...
    if (!text) { setEditingTaskId(null); return; }
    setTasks(prev => prev.map(t => t.id === id ? { ...t, text } : t));
    setEditingTaskId(null);
    await createClient().from("tasks").update({ text }).eq("id", id);
  }
            ...
            <div className="p-4">
              ...
              <div className="space-y-0.5">
                {undone.map(t => {
                  ...
                  return (
                    <div key={t.id} className="group flex items-center gap-2.5 px-2 py-2.5 rounded-xl transition-colors"
                      ...
                      {isEditing ? (
                        <>
                          <input
                            ...
                            onKeyDown={e => {
                              if (e.key === "Enter") { e.preventDefault(); saveTaskEdit(t.id); }
                              if (e.key === "Escape") { e.preventDefault(); cancelTaskEdit(); }
                            }}
                            onBlur={() => saveTaskEdit(t.id)}
                          />
                          <button onClick={cancelTaskEdit} className="text-xs flex-shrink-0 px-1" style={{ color: "var(--text-2)" }}>✕</button>
                        </>
                      ) : (
                        <>
                          ...
                          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                            <button onClick={() => startEditTask(t.id, t.text)} className="p-1 rounded hover:opacity-70" style={{ color: "var(--text-2)" }} title="Edit">
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" />
                                <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" />
                              </svg>
                            </button>
                            <button onClick={() => deleteTask(t.id)} className="p-1 rounded hover:opacity-70" style={{ color: "var(--text-2)" }} title="Delete">
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              {done.length > 0 && (
                <div className="space-y-0.5 border-t pt-2 mt-2" style={{ borderColor: "var(--border-2)" }}>
                  {done.map(t => (
                    <div key={t.id} className="group flex items-center gap-2.5 px-2 py-2.5 rounded-xl opacity-55">
                      <button onClick={() => toggleTask(t.id)}
                      ...
                      <span className="text-xs font-mono flex-shrink-0 tabular-nums mr-1" style={{ color: "var(--text-2)" }}>{formatTime(t.timeSpent)}</span>
                      <button onClick={() => deleteTask(t.id)} className="p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity hover:opacity-70 flex-shrink-0" style={{ color: "var(--text-2)" }} title="Delete">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                          <polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6" />
                          <path d="M10 11v6M14 11v6" /><path d="M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2" />
                        </svg>
