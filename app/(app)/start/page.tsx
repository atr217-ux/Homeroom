        ...
        {(scheduleOnly || scheduleMode === "later") && (
          <div className="space-y-3">
            <div className="relative" ref={calendarRef}>
              ...
              <button type="button"
                className="w-full flex items-center gap-3 border-2 rounded-xl px-3 py-2.5"
                style={{ borderColor: "var(--purple)", background: "var(--purple-bg-2)" }}
                onClick={() => setShowCalendar(v => !v)}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--purple)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                  <rect x="3" y="4" width="18" height="18" rx="2" />
                  <line x1="16" y1="2" x2="16" y2="6" />
                  <line x1="8" y1="2" x2="8" y2="6" />
                  <line x1="3" y1="10" x2="21" y2="10" />
                </svg>
                <span className="text-base font-semibold flex-1 text-left" style={{ color: scheduleDate ? "var(--purple)" : "var(--text-3)" }}>
              ...
              {showCalendar && (() => {
                ...
                return (
                  <div className="absolute z-50 left-0 right-0 mt-1 rounded-2xl p-3 shadow-xl border" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
                    ...
                    <div className="grid grid-cols-7 mb-1">
                      {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => (
                        <span key={d} className="text-center text-xs font-medium py-1" style={{ color: "var(--text-3)" }}>{d}</span>
                      ))}
                    </div>
                    <div className="grid grid-cols-7 gap-y-1">
                      ...
                      {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                          <button type="button" key={day} disabled={isPast}
                            onClick={() => { setScheduleDate(ds); setShowCalendar(false); }}
                            className="h-9 w-full flex items-center justify-center rounded-lg text-sm font-medium transition-colors"
                            style={isSelected
                              ? { background: "var(--purple)", color: "white" }
                              : isPast
                              ? { color: "var(--border-3)", cursor: "not-allowed" }
                              : { color: "var(--text)" }}>
                            {day}
                          </button>
                        );
